...
// Lê a lista de studios das variáveis de ambiente STUDIO_1_NOME / STUDIO_1_DEVICE_ID,
// STUDIO_2_NOME / STUDIO_2_DEVICE_ID, etc. Basta definir um par a mais no Vercel
// para adicionar um novo studio, sem mexer no código.
function getStudios() {
  const studios = [];
  for (let i = 1; i <= 20; i++) {
    const nome = process.env[`STUDIO_${i}_NOME`];
    const deviceId = process.env[`STUDIO_${i}_DEVICE_ID`];
    if (nome && deviceId) {
      studios.push({ id: String(i), nome, deviceId });
    }
  }
  return studios;
}

function getDeviceIdPorStudio(studioId) {
  const studios = getStudios();
  const encontrado = studios.find((s) => s.id === String(studioId));
  if (!encontrado) {
    throw new Error("Studio não encontrado. Verifique as variáveis STUDIO_N_NOME / STUDIO_N_DEVICE_ID.");
  }
  return encontrado;
}
...
async function getPasswordTicket(accessToken, deviceId) {
...
// A ticket_key vem cifrada usando o client secret como chave.
...
function gerarSenhaNumerica(tamanho = 7) {
  let senha = "";
...
async function limparSenhasExpiradas(accessToken, deviceId) {
  try {
    const lista = await tuyaFetch(
      "GET",
      `/v1.0/devices/${deviceId}/door-lock/temp-passwords?valid=true`,
      { accessToken }
    );
    ...
    for (const p of expiradas) {
      ...
      try {
        await tuyaFetch(
          "DELETE",
          `/v1.0/devices/${deviceId}/door-lock/temp-password/${id}`,
          { accessToken }
        );
      } catch (e) {
...
async function criarSenhaTemporaria({ nomeHospede, effectiveTime, invalidTime, senhaEscolhida, studioId }) {
  const studio = getDeviceIdPorStudio(studioId);
  const deviceId = studio.deviceId;

  const accessToken = await getAccessToken();
  const { ticket_id, ticket_key } = await getPasswordTicket(accessToken, deviceId);
  ...
  await limparSenhasExpiradas(accessToken, deviceId);
  ...
  return { senha: senhaFinal, resultado, nomeStudio: studio.nome };
}

module.exports = { criarSenhaTemporaria, getStudios };
